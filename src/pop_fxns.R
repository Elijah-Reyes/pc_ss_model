...
## based only on total adult population. Brood size is density dependent.
...
## ~~ Mortality ~~~~~~~~~~~~~~
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Function to figure out which adults die. Adults in
...
mortality <- function(pp, prms){
  ## Prevents model from breaking if the pop is extinct
  if(nrow(pp) < 1){return(integer(0))}
  
  ## Get the sex specific phenotypes
  ...
  ## calculate mortality and draw which individuals died
...
mortality.state <- function(pp, prms){
    if(nrow(pp) < 1){return(integer(0))}
    ...
    mort.trait <- diag(pp[,sex.phenos[pp[,'state']]])
    baseline.trait.mort <- 0.05 * (1 + (mort.trait/prms$L)^(1.5)) 
    surv.prob <- 1 - baseline.trait.mort 
    surv.ind <- rbinom(nrow(pp), 1, prob=surv.prob)
...
## use chosen mating system to figure out who mates
## For mate system CTM:
  if(prms$mate.system == 'limited.sex'){
...
## For mate system MPM:  
    if(prms$mate.system == 'limited.choosy'){
        if(male.biased){
            Nf.in <- length(f.ind)
...
## For mate system PDM:  
    if(prms$mate.system == 'choosy.sex'){
       f.ind <- f.ind[order(ff[f.ind,'pheno.sexf'], decreasing=T)]
       m.ind <- m.ind[order(mm[m.ind,'pheno.sexm'], decreasing=T)] 
       mate.mat <- outer(ff[f.ind,'pheno.sexf']/prms$L, mm[m.ind,'pheno.sexm']/prms$L)
       if(male.biased){
        n.matings <- length(f.ind)
        mates <- vector(length=n.matings)
            for(ii in 1:n.matings){
                if(sum(mate.mat[ii,]) == 0){mate.mat[ii,] <- 1}
                mm.mate <- sample(1:ncol(mate.mat), 1, prob=mate.mat[ii,])     
                mates[ii] <- m.ind[mm.mate]
                m.ind <- m.ind[-mm.mate]
                mate.mat <- mate.mat[,-mm.mate, drop=FALSE]
            }        
        moms <- f.ind
        dads <- mates 
       }else{
        n.matings <- length(m.ind)
        mates <- vector(length=n.matings)
            for(ii in 1:n.matings){
                if(sum(mate.mat[,ii]) == 0){mate.mat[,ii] <- 1}
                ff.mate <- sample(1:nrow(mate.mat), 1, prob=mate.mat[,ii])     
                mates[ii] <- f.ind[ff.mate]
                f.ind <- f.ind[-ff.mate]
                mate.mat <- mate.mat[-ff.mate,, drop=FALSE]
            }        
        moms <- mates
        dads <- m.ind
       }
...
## also assign a sex here. At maturity, juveniles may disperse. 
...
update.adults <- function(pop=pop, prms=prms, coffin=coffin, matured, gen){
  ...
  ## ~~ Mutate ~~~~~
  ## ~~~~~~~~~~~~~~~
    ## Only mutates the parental care and sexual traits
    loci <- 1:(prms$L*4)
    if(prms$ss.off == T){
  ...
  ## store and pass updated population in main environment
...
## updates fitness storage object
...
## updates object that stores individuals that are dead but have immature offspring
...
## updates object that stores information on how long each individual lived 
